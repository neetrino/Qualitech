...
import { blogDetailQuerySchema, blogSlugParamSchema } from "@/features/blog/blog.schemas";
...
export async function GET(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  const slugParsed = blogSlugParamSchema.safeParse({ slug: rawSlug });
  ...
  const queryParsed = parseSearchParams(blogDetailQuerySchema, searchParams);
  if (!queryParsed.ok) {
    return queryParsed.response;
  }

  try {
    const data = await getBlogPostBySlugPublic(slugParsed.data.slug, queryParsed.data.locale);
    if (!data) {

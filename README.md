...
1. Պատճենիր `.env.example` → `.env.local`, լրացրու `DATABASE_URL` (Neon)։
2. Տեղադրում. `pnpm install` (կամ `npm install`, եթե pnpm չկա)։
3. Prisma. `npx prisma generate` (կատարվում է `postinstall`-ով)։
4. Բազա. `pnpm db:migrate` (կամ dev-ում `pnpm db:migrate:dev`), ապա `pnpm db:seed` — մինիմալ կատեգորիա, հաստոց, բլոգ։
5. Dev. `pnpm dev` — բացի [http://localhost:3000](http://localhost:3000)։
6. Առողջության ստուգում. `GET /api/health` — `db: up` եթե ԲԴ կապը կա։
7. Թեստեր. `npm test` (կամ `pnpm test`) — Vitest unit smoke։

Փաստաթղթեր. `docs/TECH_CARD.md`, `docs/01-ARCHITECTURE.md`, `docs/BACKEND_IMPLEMENTATION_PLAN.md`, `docs/API.md`։
